    ...
    T::IntoIter: ExactSizeIterator,
{
    ...
    let mut bins = GoertzelBin::apply_goerzel(
        samples.into_iter(),
        sample_rate,
...
impl GoertzelBin {
        ...
        F: Iterator<Item = u16>,
    {
        ...
        let mut bins: Vec<_> = freqs
            .map(|freq| GoertzelBin::new(freq, step, len))
            .collect();
    ...
    pub fn add_sample(&mut self, sample: f64) {
        self.coeff = (
            sample + self.real * self.coeff.0 - self.coeff.1,
            self.coeff.0,
        );
    }
